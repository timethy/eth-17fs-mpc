...
\paragraph{$l_\infty$}
$\min \lVert x \rVert_\infty$ equivalent to $\min_{x \in \R^n} \left[ \max\{x,\dots,x_n,-x_1,\dots,-x_n\}\right]$. \\
Epigraph gives:
\tim{Set this in better form}
$\min_{x,t} t$ such that $x_i \leq t, -x_i \leq t, \vF x \leq g$ $\iff$ $\min_{x,t} t$ such that $-{\bm 1} t \leq x \leq 1 t, \vF_x \leq g$.
